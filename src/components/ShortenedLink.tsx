...
export default function ShortenedLink({alias}: ShortenedLinkProps) {
    ...
    return (
        <motion.div key={alias}
            ...
            <div
                className="mb-3 flex items-center gap-2 rounded-xl bg-green-400 p-4 text-gray-700 transition hover:bg-green-500">
                <FiCheck/>
                <h2>Successfully generated link!</h2>
                <CopyButton text={link}/>
            </div>
            <Link href={link} target="_blank"
                  className="mb-5 text-indigo-700 underline transition hover:text-violet-700 dark:text-indigo-500 dark:hover:text-indigo-400">
                {link}
            </Link>
